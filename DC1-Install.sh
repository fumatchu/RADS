...
prompt_static_ip_if_dhcp() {
  ...
  return
    ...
    while true; do
      ...
      while true; do
        DNSSERVER=$(dialog --backtitle "Interface Setup" --title "DNS Server" --inputbox "(Enter the Primary AD Server IP address)\nEnter DNS server IP:" 8 60 3>&1 1>&2 2>&3)
...
update_and_install_packages() {
  ...
  sleep 2
  PACKAGE_LIST=("ntsysv" "iptraf" "nano" "expect" "rsync" "sshpass" "openldap-clients" "fail2ban" "tuned" "createrepo" "cockpit" "cockpit-storaged" "mock" "cockpit-files" "net-tools" "dmidecode" "ipcalc" "bind-utils"  "iotop" "zip" "yum-utils" "nano" "curl" "wget" "git" "dnf-automatic" "dnf-plugins-core" "util-linux" "htop" "iptraf-ng" "mc")
  TOTAL_PACKAGES=${#PACKAGE_LIST[@]}
...
EOF
  ...
  if systemctl is-active --quiet dhcpd; then
    ...
    dialog --backtitle "DHCP Server Setup" --msgbox "The DHCP service is NOT running.\nPlease validate your configuration before expecting DHCP to service clients." 8
 60
  fi
}
#===========SET SELINUX=============
...
setup_samba_ldaps_cert() {
  ...
  dialog --backtitle "Configuring TLS" --title "Samba TLS Setup" --infobox "Generating Samba LDAPS certificate for $FQDN with IP $IPADDR..." 6 100
  sleep 2
