#!/bin/sh
#DC1-install.sh
#This script installs Samba AD (Secondary/Tertiary Server) with DC support using mock from Upstream Rocky REPO via src.rpm
...
USER=$(whoami)
DHCPNSNAME=$(hostname | sed 's/^[^.:]*[.:]//' |sed -e 's/\(.*\)/\U\1/')
...
EOF
sleep 4s
dnf -y install ntsysv open-vm-tools cockpit cockpit-storaged net-tools nano
dnf -y update
# Initial build
